...
public class Main {
	...
	public static void main(String[] arguments) throws URISyntaxException {
		...
		ctx.put("_mainClass", Main.class); //used to access Java class features
		
		if (debug) {
			executor.startDebugger(debugSettings);
		}
		
		executor.execute(main, ctx);
		
		//We shouldn't stop debugger in this thread because AWT creates it's own
		//event-dispatching thread and this thread stops before exit
